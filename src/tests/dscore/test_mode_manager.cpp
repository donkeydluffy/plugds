...
class MockMode : public sss::dscore::IMode {
 ...
 public:
  MockMode(QString id, QString title, int ctx_id) : id_(std::move(id)), title_(std::move(title)), ctxId_(ctx_id) {}
  ...
  [[nodiscard]] int ContextId() const override { return ctxId_; }
...
class MockWorkbench : public QObject, public sss::dscore::IWorkbench {
 ...
 public:
  ...
  void SetBackgroundWidget(QWidget* widget) override { (void)widget; }
  ...
  void SetActiveModeButton(const QString& id) override { (void)id; }
  void SetModeSwitchCallback(std::function<void(const QString&)> callback) override { (void)callback; }
};
...
struct ModeManagerFixture {
  ...
  sss::dscore::ModeManager* mode_mgr = nullptr;           // NOLINT
  MockWorkbench* mock_workbench = nullptr;                // NOLINT
...
TEST_SUITE("ModeManager") {
  TEST_CASE_FIXTURE(ModeManagerFixture, "Add and Activate Mode") {
    auto* mode1 = new MockMode("mode1", "Mode 1", 10);
    auto* mode2 = new MockMode("mode2", "Mode 2", 20);

    QSignalSpy spy_added(mode_mgr, &sss::dscore::ModeManager::ModeAdded);
    QSignalSpy spy_changed(mode_mgr, &sss::dscore::ModeManager::ModeChanged);

    mode_mgr->AddMode(mode1);
    CHECK(mode_mgr->Modes().size() == 1);
    CHECK(spy_added.count() == 1);

    mode_mgr->AddMode(mode2);
    CHECK(mode_mgr->Modes().size() == 2);
    CHECK(spy_added.count() == 2);
